...
public class EmployeeController {
    ...
    public ResponseEntity<Employee> updateEmployeeById(@PathVariable Long id,
            @Valid @RequestBody UpdateEmployeeDTO employeeData) throws Exception {
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<Employee> deleteEmployeedById(@PathVariable Long id) throws Exception {
        Optional<Employee> result = this.employeeService.deleteEmployeeById(id);
        Employee deletedEmployee = result
                .orElseThrow(() -> new NotFoundException("Could not find employee with id " + id));
